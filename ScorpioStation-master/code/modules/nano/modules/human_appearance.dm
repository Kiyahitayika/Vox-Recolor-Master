...
/datum/nano_module/appearance_changer/Topic(ref, href_list, var/nowindow, var/datum/topic_state/state = GLOB.default_state)
	...
	if(href_list["skin_tone"])
		if(can_change_skin_tone())
			...
			if(owner.dna.species.bodyflags & HAS_SKIN_TONE)
			...
			else if(owner.dna.species.bodyflags & HAS_LOCKED_SKIN_COLOR) //Iren Code
				...
				if(isnum(new_s_tone) && can_still_topic(state))
					...
					var/new_skin = owner.dna.species.getspeciescolor(new_s_tone) //Iren Code - get that color // Iren Code
					owner.change_skin_color(new_skin)
					update_dna()
					//owner.dna.species.
...
/datum/nano_module/appearance_changer/ui_data(mob/user, ui_key = "main", datum/topic_state/state = GLOB.default_state) //Iren Code -- look here
...
/datum/nano_module/appearance_changer/proc/can_change_skin_tone() //Iren Code Below
	return owner && (flags & APPEARANCE_SKIN) && ((owner.dna.species.bodyflags & HAS_SKIN_TONE) || (owner.dna.species.bodyflags & HAS_LOCKED_SKIN_COLOR))

/datum/nano_module/appearance_changer/proc/can_change_skin_color()	//Iren Code Below
	return owner && (flags & APPEARANCE_SKIN) && ((owner.dna.species.bodyflags & HAS_SKIN_COLOR) || (owner.dna.species.bodyflags & HAS_LOCKED_SKIN_COLOR))
