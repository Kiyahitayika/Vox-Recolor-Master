...
/mob/living/carbon/human/proc/set_species(datum/species/new_species, default_colour, delay_icon_update = FALSE, skip_same_check = FALSE, retain_damage = FALSE)
	...
	if(dna.species.base_color && default_colour) //Iren Code
		//Apply colour.
	...
	if(dna.species.bodyflags & HAS_LOCKED_SKIN_COLOR)	//Iren Code
		s_tone = dna.species.getspeciestone(skin_colour)
	else if(!(dna.species.bodyflags & HAS_SKIN_TONE) && !(dna.species.bodyflags & HAS_LOCKED_SKIN_COLOR))
		s_tone = 0
