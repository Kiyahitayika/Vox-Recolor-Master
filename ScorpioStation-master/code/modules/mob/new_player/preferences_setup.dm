...
/datum/preferences/proc/update_preview_icon(var/for_observer=0)		//seriously. This is horrendous.
	...
	icobase = current_species.icobase // Fix
	//icobase = 'icons/mob/human_races/r_human.dmi'
	//Icon-based species colour.
	var/coloured_tail
	//if(current_species)
	//Iren - work here
		//if(current_species.bodyflags & HAS_LOCKED_SKIN_COLOR) //Handling species-specific icon-based skin tones by flagged race.
	...
	for(var/name in list("chest", "groin", "head", "r_arm", "r_hand", "r_leg", "r_foot", "l_leg", "l_foot", "l_arm", "l_hand"))
		...
		// if((current_species.bodyflags & HAS_LOCKED_SKIN_COLOR && (name == "l_foot" || name == "r_foot" || name == "l_hand" || name == "r_hand"))) //Iren Code - don't color the pre-colored Vox hands and feet
		// 	preview_icon.Blend(new /icon(icobase, "[name]"), )
		// else
		// 	preview_icon.Blend(new /icon(icobase, "[name]"), ICON_OVERLAY)
	...
	else if(current_species && (current_species.bodyflags & HAS_LOCKED_SKIN_COLOR)) //Iren Code
		if(s_tone >= 0)
			preview_icon.Blend(s_colour, ICON_ADD)

	//Tail
	if(current_species && (current_species.bodyflags & HAS_TAIL)) //Iren Code look here
		var/tail_icon
		var/tail_icon_state
		var/tail_shift_x
		var/tail_shift_y
