...
/obj/item/organ/external/proc/sync_colour_to_human(var/mob/living/carbon/human/H)
	...
	if(dna.unique_enzymes != H.dna.unique_enzymes) // This isn't MY arm
		if(!(H.dna.species.bodyflags & HAS_ICON_SKIN_TONE))
			sync_colour_to_dna()
			return
	if(!isnull(H.s_tone) && (H.dna.species.bodyflags & HAS_SKIN_TONE)) // IREN CODE
		s_col = null
		s_tone = H.s_tone
	if(H.dna.species.bodyflags & HAS_SKIN_COLOR)
		s_col = H.skin_colour
		s_tone = null
	if(H.dna.species.bodyflags & HAS_LOCKED_SKIN_COLOR)	//Iren Code
		s_col= H.skin_colour
		//rgb(H.dna.GetUIValue(DNA_UI_SKIN_R), H.dna.GetUIValue(DNA_UI_SKIN_G), H.dna.GetUIValue(DNA_UI_SKIN_B))
		//s_tone = H.dna.species.getspeciestone(s_col)
		s_tone = null

/obj/item/organ/external/proc/sync_colour_to_dna()
	if(is_robotic())
		return
	if(!isnull(dna.GetUIValue(DNA_UI_SKIN_TONE)) && (dna.species.bodyflags & HAS_SKIN_TONE)) //Iren Code
		s_col = null
		s_tone = dna.GetUIValue(DNA_UI_SKIN_TONE)
	if(dna.species.bodyflags & HAS_SKIN_COLOR)	//Iren Code ADD A BEAK SOMEWHERE OMG
		s_col = rgb(dna.GetUIValue(DNA_UI_SKIN_R), dna.GetUIValue(DNA_UI_SKIN_G), dna.GetUIValue(DNA_UI_SKIN_B))
		s_tone = null
	if(dna.species.bodyflags & HAS_LOCKED_SKIN_COLOR)	//Iren Code
		s_col= rgb(dna.GetUIValue(DNA_UI_SKIN_R), dna.GetUIValue(DNA_UI_SKIN_G), dna.GetUIValue(DNA_UI_SKIN_B))
		s_tone = dna.species.getspeciestone(s_col)
		//message_admins("Sync Colour to DNA Colour: " s_col)
		//message_admins("Sync Colour to DNA Tone: " s_tone)
...
/obj/item/organ/external/proc/get_icon(skeletal)
	...
	if(force_icon)
		...
		if(!skeletal && !is_robotic())
			...
			if(!isnull(s_tone) && !(dna.species.bodyflags & HAS_LOCKED_SKIN_COLOR))	//Iren Code
				if(s_tone >= 0)
