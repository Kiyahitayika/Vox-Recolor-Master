...
/datum/preferences/proc/ShowChoices(mob/user)
	...
	switch(current_tab)
		if(TAB_CHAR) // Character Settings
			...
			if(S.bodyflags & (HAS_SKIN_TONE|HAS_LOCKED_SKIN_COLOR)) //Iren Code
				dat += "<b>Skin Tone:</b> <a href='?_src_=prefs;preference=s_tone;task=input'>[S.bodyflags & HAS_LOCKED_SKIN_COLOR ? "[s_tone]" : "[-s_tone + 35]/220"]</a><br>"
			...
			if(!(S.bodyflags & ALL_RPARTS))
				...
				dat += "<a href='?_src_=prefs;preference=eyes;task=input'>Color</a> [color_square(e_colour)]<br>"
																	...
																	//Iren Code Below v
			if(((S.bodyflags & HAS_SKIN_COLOR) && !(S.bodyflags & HAS_LOCKED_SKIN_COLOR)) || (GLOB.body_accessory_by_species[species] && !(S.bodyflags & HAS_LOCKED_SKIN_COLOR)) || check_rights(R_ADMIN, 0, user)) //admins can always fuck with this, because they are admins
				dat += "<b>Body Color:</b> "
...
//								alert(user, "You cannot change your species as you need to be whitelisted. If you wish to be whitelisted contact an admin in-game, on the forums, or on IRC.")
					...
					if(prev_species != species)
						...
						if(NS.bodyflags & HAS_SKIN_TONE)//Iren Code
...
*/
				...
				if("s_tone")
					if(S.bodyflags & HAS_SKIN_TONE)
					...
					else if(S.bodyflags & HAS_LOCKED_SKIN_COLOR) //Iren Code
						...
						if(isnum(skin_c))
							...
							s_colour = S.getspeciescolor(skin_c) //Iren Code - get that color
							//updatespeciescolor(user, 0)
...
/datum/preferences/proc/copy_to(mob/living/carbon/human/character)
	...
	if(!(S.bodyflags & HAS_LOCKED_SKIN_COLOR)) //Iren Code - GOES HERE
		character.s_tone = s_tone
		character.skin_colour = s_colour
	else
		character.s_tone = s_tone
		character.skin_colour = S.getspeciescolor(s_tone) //Iren Code - get that color
		//message_admins(character.s_tone)
		//message_admins(s_colour)
